...
//! but the things worth testing here — schema application, an advisory lock, a trigger that raises
//! — behave differently inside one, and a suite that cannot observe a trigger firing is not testing
//! the schema it claims to test.
...
/// `max_connections` long before the tests are slow.
const TEST_POOL_SIZE: u32 = 2;

/// Where the disposable databases are created.
///
/// `TELEGRAM_TEST_DATABASE_URL` overrides it; the default matches `compose.yaml`, so `docker compose
...
#[expect(
    ...
    reason = "the workspace bans direct environment reads so that configuration has exactly one \
              ...
              reads a variable that is not part of the Telegram configuration at all: it names \
              where a test may create and drop databases."
)]
pub fn admin_url() -> String {
    env::var("TELEGRAM_TEST_DATABASE_URL")
...
impl TestDatabase {
    ...
    pub async fn create() -> Result<Self, PersistenceError> {
        let name = format!("telegram_test_{}", Uuid::now_v7().simple());
        let admin = admin_url();
        ...
        // `compose.yaml` and CI, the runner's glibc otherwise. Two collations across the two places
        // that check each other is how a text index sorts one way where it is built and another
        // where it is read.
        pool.execute(
            format!(
