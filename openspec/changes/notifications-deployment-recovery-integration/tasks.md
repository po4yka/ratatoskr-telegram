...
- [ ] 9.3 Run workspace TG-010 against the exact Platform and Telegram commits and verify plan-item-5 article completion, enabled notification delivery, disabled suppression, duplicate dedupe, and task-namespaced cleanup; record this as synthetic composed evidence, not live Telegram/provider proof.
- [ ] 9.4 Run `openspec validate notifications-deployment-recovery-integration --strict`, review the final diff for schema-version/migration violations, unsafe recovery, secret leakage, unrelated changes, and stale call sites, then mark tasks complete only after their named evidence was observed.
